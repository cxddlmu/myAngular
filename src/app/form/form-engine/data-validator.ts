...
//   constructor(private domainObj, private formGroup: FormGroup, private formConfig?, private metadata?: Metadata) {}
...
//       if (staticMandatoryFields && staticMandatoryFields.indexOf(controlKey) > -1) {
//         //mandatory==true
...
//     for (let controlKey in this.formConfig) {
//       let settingEntity = this.formConfig[controlKey];
//       if (staticMandatoryFields && staticMandatoryFields.some(x => x.indexOf(controlKey) > -1)) {
//         //mandatory==true
//         this.setIsOptional(controlKey, false);
//         this.formGroup.get(controlKey);
//       } else {
...
//         let conditionalMandatoryVals = conditionalMandatoryMap[conditionalMandatoryKey] as Array<any>;
//         let conditionalMandatoryKeySplitArr = conditionalMandatoryKey.split(this.separator);
...
//             let result = this.executeCompletenessConditionsForObj(conditionsMandatoryKey);
...
//                 let result = this.executeCompletenessConditionsForArr(conditionalMandatoryVal, domainValIndex, conditionalMandatoryKeySplitArr[0]);
//                 let settingEntity = this.formConfig[conditionalMandatoryKey];
//                 if (result == true) {
//                   if (!settingEntity["isOptionals"]) {
//                     settingEntity["isOptionals"] = [];
...
//     }
//   }
//   private executeCompletenessConditionsForArr(conditionsMandatoryKey, index: number, key): boolean {
//     let conditionsMandatoryMap = this.metadata.getCompletenessMetadata().conditions;
//     if (!conditionsMandatoryMap) {
//       return false;
//     }
//     let conditionsMandatoryVal = conditionsMandatoryMap.get(conditionsMandatoryKey);
//     let compiledConditionsMandatoryVal = this.executeCompletenessRegex(conditionsMandatoryVal);
//     if (index != undefined) {
//       let cnt = this.domainObj[key].length; //calculate array length
//       for (let index = 0; index < cnt; index++) {
//         //eval all the condition in array
//         if (typeof this.domainObj != "undefined" && compiledConditionsMandatoryVal.indexOf("undefined") != 0) {
//           if (key) {
...
//     return conditionsMandatoryVal.replace(new RegExp(/{([^}]*)}/, "g"), (match, token) => {
...
//           return `this.formConfig.${tokens[0]}.options.find(function(item){return item.key=="${value}";}) && this.formConfig.${tokens[0]}.options.find(function(item){return item.key=="${value}";}).${
//             tokens[1]
//           }`;
//         }
//       }
...
//   private validateCompletenetssForObj(formControlKey: string, formGroup: FormGroup): void {
//     formGroup.get(formControlKey).updateValueAndValidity({ onlySelf: true, emitEvent: false });
...
//     }
//   }
//   private validateCompletenetssForArr(formControlKey: string, formArrayKey: string, index: number, formGroup: FormGroup): void {
//     (formGroup.get(formArrayKey) as FormArray)
//       .at(index)
//       .get(formControlKey)
//       .updateValueAndValidity({ onlySelf: true, emitEvent: false });
//     let compiledConditionsMandatoryVal = this.executeCompletenessRegex(formArrayKey);
//     if (index != undefined) {
//       let cnt = this.domainObj[key].length; //calculate array length
//       for (let index = 0; index < cnt; index++) {
//         //eval all the condition in array
...
//     }
//   }
//   public validateCompleteness(index?, controlKeyParent?, formGroup?: FormGroup): Array<any> {
...
//         let staticMandatoryFieldSplits = staticMandatoryField.split(this.separator) as Array<any>;
//         let formControlKey;
//         let formArrayKey;
...
//                 }
//                 // check field value
//                 if (!this.domainObj[fieldName] || (this.domainObj[fieldName] && this.domainObj[fieldName].length == 0)) {
...
//             } else if (_fieldName.length == 2) {
//               if (typeof this.domainObj != "undefined" && typeof this.domainObj[_fieldName[0]] != "undefined") {
//                 (<Array<any>>this.domainObj[_fieldName[0]]).forEach((value, index_2, a) => {
//                   //array iterate
//                   // 2. execute condition expressions
//                   let indexParam;
//                   if (index) {
...
//                   }
//                   let executedConditions = this.executeCompletenessConditions(indexParam, domainField);
//                   if (executedConditions[requiredConditions[i]] && executedConditions[requiredConditions[i]][index_2] && executedConditions[requiredConditions[i]][index_2] == true) {
